...
export class SdkService {
  ...
  public static getOrCreateRoom(address: string) {
    this.ensureInitialized();
    return this.sdk?.chat.getOrCreateRoom({
      users: [address],
      parameters: null,
    });
  }

  public static sendMessage(roomid: string, message: string) {
    this.ensureInitialized();
    return this.sdk?.chat.send({
      roomid,
      content: {
        messages: [message],
      },
